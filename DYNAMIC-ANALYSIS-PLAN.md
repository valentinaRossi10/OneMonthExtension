...
| CVE-2021-42386 (awk) | `nvalloc` is `static`, but `awk_main` (self-contained, externally visible) is the natural entry point | None needed at the entry level; turned out to need **only** argv fuzzing (whole script as one positional arg) — the earlier "needs file-content fuzzing too" guess was wrong, corrected once built: the bug is reachable purely from a `BEGIN` block | **Built** — `dynamic-analysis/cve-2021-42386/`; crash not yet reproduced (discovery campaign found 21 crashes, none the target UAF — see PROVENANCE.md) |
