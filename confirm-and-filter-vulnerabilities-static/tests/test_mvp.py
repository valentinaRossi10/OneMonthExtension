...
class MvpTest(unittest.TestCase):
            ...
            [
                {
                    ...
                    "analysis_source": "ghidra-program-model-static-v2",
    ...
    def test_force_include_is_exact_and_does_not_forward_other_negatives(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            scoring = root / "runs" / "run-a" / "scoring" / "scoring.csv"
            base = {
                "cve_id": "CVE-X",
                "variant": "vulnerable",
                "indexed_bug_class": "use-after-free",
                "target_class": "use-after-free",
                "expected_positive": "True",
                "input_status": "ready",
                "response_status": "ok",
                "model_verdict": "not_vulnerable",
                "confidence": "0.9",
                "evidence_lines": "[]",
                "summary": "negative",
                "model": "test",
            ...
            self.write_scoring(
                scoring,
                [
                    {**base, "task_id": "known-false-negative"},
                    {**base, "task_id": "other-negative"},
                ],
            )
            bindings = root / "bindings.jsonl"
            jsonl(
                ...
                [
                    {
                        ...
                        "function_uid": "fn-" + suffix * 32,
                        "package_path": "package",
                        "entry_roots": [],
                    }
                    for task_id, suffix in (
                        ("known-false-negative", "1"),
                        ("other-negative", "2"),
                    )
                ],
            )
            ...
            self.assertEqual(
                queue[0]["source_rows"][0]["source_task_id"],
                "known-false-negative",
            )
            self.assertTrue(queue[0]["source_rows"][0]["forced_inclusion"])
            summary = json.loads((output / "ingestion-summary.json").read_text())
            self.assertEqual(summary["forced_inclusion_count"], 1)
            self.assertEqual(summary["raw_row_count"], 2)
            self.assertEqual(summary["eligible_raw_row_count"], 1)
    ...
    def test_package_uses_analyzed_calls_and_blocks_unreachable_suppression(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            indirect = package.execute_tool(
                "resolve_indirect_call",
                {
                    "function_uid": caller["function_uid"],
                    "site_address": "2008",
                },
            )
            self.assertEqual(indirect["resolution_status"], "unresolved")
            dominance = package.execute_tool(
                "get_dominance",
                {
            ...
            self.assertEqual(dominance["entry_blocks"], ["1000"])
            task = {
                "target_class": "null-pointer-dereference",
                "function_uid": target["function_uid"],
