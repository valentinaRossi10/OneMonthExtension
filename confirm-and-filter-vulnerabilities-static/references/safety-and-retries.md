...
For the reviewed round-3 policy, the runner also ensures that every provider
request—including the first tool-enabled request—ends with the same defensive
symbolic-only reminder. This applies uniformly to all cases in that scope. It
reduces refusal risk but cannot guarantee that the provider will accept a
request.
...
The reviewed round-3 policy has one explicit scheduling exception: if a
required tool returns `tool_result_too_large`, grant the frozen adaptive stage
immediately so the agent can narrow and retry it without starving the other
required tools. The failed call remains recorded and counted; the exception
does not increase the manifest's absolute model/tool-call or monetary limits,
and it does not apply to optional tools or other errors.
...
## Static backend limits

Ghidra-backed def-use, slicing, dominance, reference, and call-path queries are
bounded intraprocedural or graph queries over frozen package indexes. They do
not silently infer memory aliases, callee effects, or loop invariants.

angr queries are local, targeted, and bounded by per-case query count, active
states, steps, and wall-clock time. Record the query arguments, query hash,
backend version/status, pruning, and limitation. Backend absence, unsupported
lifting, timeout, state pruning, unconstrained successors, or failure to find a
target within bounds always yields `retain_and_escalate` unless an independent
complete positive proof already exists. Never retry by broadening into
