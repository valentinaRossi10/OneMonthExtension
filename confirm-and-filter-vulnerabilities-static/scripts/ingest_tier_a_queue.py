...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument(
        ...
        action="append",
        default=[],
        metavar="ROW_SELECTOR",
        help=(
            "Forward one reviewed row regardless of model_verdict. A selector "
            "must uniquely match source_row_id, source task_id, or "
            "source_run_id:source_task_id. Repeat for multiple rows."
        ),
    )
    return value
...
def main() -> int:
    ...
    raw_row_count = 0
    raw_rows: list[tuple[dict[str, Any], dict[str, Any] | None]] = []
    selector_matches: dict[str, list[str]] = {
        selector: [] for selector in args.force_include
    }
    if len(selector_matches) != len(args.force_include):
        raise ValueError("Duplicate --force-include selector")
    for scoring_path in args.scoring_csv:
        ...
        with scoring_path.open(newline="", encoding="utf-8") as handle:
            for row_number, row in enumerate(csv.DictReader(handle), 2):
                ...
                raw_rows.append((source, binding))
                candidates = {
                    source["source_row_id"],
                    source["source_task_id"],
                    f"{source['source_run_id']}:{source['source_task_id']}",
                }
                for selector in selector_matches:
                    if selector in candidates:
                        selector_matches[selector].append(source["source_row_id"])
    invalid_selectors = {
        selector: matches
    ...
    if invalid_selectors:
        details = ", ".join(
            f"{selector!r} matched {len(matches)} rows"
            for selector, matches in sorted(invalid_selectors.items())
        )
        raise ValueError(f"Every --force-include selector must match exactly once: {details}")
    forced_row_to_selector = {
        matches[0]: selector for selector, matches in selector_matches.items()
    }

    eligible: list[tuple[dict[str, Any], dict[str, Any] | None, str | None]] = []
    for source, binding in raw_rows:
        selector = forced_row_to_selector.get(source["source_row_id"])
        forced = selector is not None
        if source["model_verdict"] not in forward_verdicts and not forced:
            continue
        source["forced_inclusion"] = forced
        source["force_include_selector"] = selector
        class_error: str | None = None
        try:
            normalized = normalize_class(source["target_class_raw"])
    ...
    summary = {
        ...
        "force_include_selectors": sorted(args.force_include),
        "forced_source_row_ids": sorted(forced_row_to_selector),
        "forced_inclusion_count": len(forced_row_to_selector),
