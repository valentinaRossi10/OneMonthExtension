...
from static_analysis_tools import api_tools as static_api_tools
from static_analysis_tools import execute as execute_static_tool
...
def api_tools(policy: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if policy.get("package_version") == "tier-b-filter-static-package-v2":
        return base_tools + static_api_tools(policy)
    return base_tools
...
class AnalysisPackage:
    ...
    def __init__(self, package_dir: Path, policy: dict[str, Any]):
        ...
        expected_metadata = (
            self.REQUIRED_METADATA_V2
            if package_version == "tier-b-filter-static-package-v2"
            else self.REQUIRED_METADATA_V1
        )
        if set(self.metadata) != expected_metadata:
        ...
        static_v2 = package_version == "tier-b-filter-static-package-v2"
        ...
        if static_v2:
            blocks_path = self.package_dir / self.metadata["blocks_index"]
            pcode_path = self.package_dir / self.metadata["pcode_index"]
            references_path = self.package_dir / self.metadata["references_index"]
            if sha256_file(blocks_path) != self.metadata["blocks_sha256"]:
                raise ValueError("Basic-block index hash mismatch")
            if sha256_file(pcode_path) != self.metadata["pcode_sha256"]:
                raise ValueError("P-code index hash mismatch")
            if sha256_file(references_path) != self.metadata["references_sha256"]:
                raise ValueError("Reference index hash mismatch")
            self.block_rows = read_jsonl(blocks_path)
            self.pcode_rows = read_jsonl(pcode_path)
            self.reference_rows = read_jsonl(references_path)
        else:
            self.block_rows = []
            self.pcode_rows = []
            self.reference_rows = []
        ...
        if static_v2:
            if len(self.block_rows) != self.metadata["block_count"]:
                raise ValueError("Basic-block count mismatch")
            if len(self.pcode_rows) != self.metadata["pcode_operation_count"]:
                raise ValueError("P-code operation count mismatch")
            if len(self.reference_rows) != self.metadata["reference_count"]:
        ...
        unresolved = sum(
            (
                row.get("call_kind") == "indirect"
                and row.get("resolution_status") != "resolved_reference"
            )
            if static_v2
            else row.get("resolution_status") == "unresolved"
            for row in self.callsite_rows
        )
        if unresolved != self.metadata["unresolved_indirect_callsite_count"]:
            raise ValueError("Unresolved indirect-call count mismatch")
        self.blocks_by_function: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for row in self.block_rows:
            if row.get("function_uid") not in self.functions:
                raise ValueError("Basic block names an unknown function")
            self.blocks_by_function[row["function_uid"]].append(row)
        self.pcode_by_function: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for row in self.pcode_rows:
            if row.get("function_uid") not in self.functions:
                raise ValueError("P-code operation names an unknown function")
            self.pcode_by_function[row["function_uid"]].append(row)
        self.references_by_function: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for row in self.reference_rows:
            if row.get("function_uid") not in self.functions:
                raise ValueError("Reference names an unknown function")
            self.references_by_function[row["function_uid"]].append(row)
    ...
    def execute_tool(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        elif name == "query_angr":
            from angr_backend import query as query_angr

            result = query_angr(
                artifact_path=str(self.artifact_path),
                analysis_image_base=str(self.metadata["analysis_image_base"]),
                query_kind=str(arguments["query_kind"]),
                start_address=str(arguments["start_address"]),
                target_address=str(arguments["target_address"]),
                avoid_addresses=[
                    str(item) for item in arguments["avoid_addresses"]
                ],
                register=str(arguments["register"]),
                ...
                max_active_states=int(
                    self.policy["limits"].get("max_angr_active_states", 64)
                ),
            )
            result["artifact_sha256"] = self.metadata["artifact_sha256"]
            result["query_sha256"] = sha256_text(canonical_json(arguments))
        else:
            result = execute_static_tool(self, name, arguments)
