...
REINFORCED_LIFETIME_CONTEXT_REMINDER = (
    DEFENSIVE_CONTEXT_REMINDER
    + " For lifetime analysis, discuss only abstract ownership, alias, release, "
    "reuse, and dereference transitions; never derive a concrete input or action."
)
...
def projection_context_reserve_chars(
    *, reinforced_lifetime: bool = False
) -> tuple[int, int]:
    """Return current-call and per-prior-call defensive-context reserves."""
    reminder = (
        REINFORCED_LIFETIME_CONTEXT_REMINDER
        if reinforced_lifetime
        else DEFENSIVE_CONTEXT_REMINDER
    )
    current = len(
        reminder + " " + FINAL_SYNTHESIS_INSTRUCTION
    )
    replayed = max(
        len(reminder),
        len(reminder + " " + V3_NO_TEXT_CONTINUATION),
        len(reminder + " " + V2_NO_TEXT_CONTINUATION),
        len(reminder + " " + V3_INTERIM_CONTINUATION),
        len(reminder + " " + V2_INTERIM_CONTINUATION),
    )
