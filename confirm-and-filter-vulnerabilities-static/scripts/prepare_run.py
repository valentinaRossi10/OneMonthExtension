...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument(
        "--reviewed-static-primary-scope",
        action="store_true",
        help=(
            "For protocol v4 only, select the exact reviewed three-case "
            "agent-directed static-analysis scope."
        ),
    )
    value.add_argument(
        ...
        help=(
            "For protocol v4 only, select the exact reviewed three-case "
            "rerun with mandatory static-tool coverage."
        ),
    )
    value.add_argument(
        ...
        action="store_true",
        help=(
            "For protocol v4 only, select the exact reviewed two-case "
            "uniform-framing and required-tool-retry follow-up."
        ),
    )
    return value
...
def build_prompt(
    ...
    template = prompt_template_path(
        protocol_version, policy["policy_version"]
    ...
    replacements = {
        ...
        "{{REQUIRED_TOOLS}}": json.dumps(
            case.get("required_tool_names", []), sort_keys=True
        ),
    }
...
def validate_static_round3_scope_source(
    scope: dict[str, Any], results_root: Path
) -> None:
    source_run_dir = results_root / scope["source_run_id"]
    manifest_path = source_run_dir / "manifest.jsonl"
    results_path = source_run_dir / "results.jsonl"
    if not manifest_path.is_file() or not results_path.is_file():
        raise ValueError("V4 round-3 source run is unavailable")
    if sha256_file(manifest_path) != scope["source_manifest_sha256"]:
        raise ValueError("V4 round-3 source manifest hash differs")
    if sha256_file(results_path) != scope["source_results_sha256"]:
        raise ValueError("V4 round-3 source results hash differs")
    source_tasks = {
        task["case_id"]: task for task in read_jsonl(manifest_path)
    }
    ...
    for scoped in scope["cases"]:
        case_id = scoped["case_id"]
        source_task = source_tasks.get(case_id)
        if source_task is None:
            raise ValueError(f"V4 round-3 source task is missing: {case_id}")
        if (
            source_task["artifact_sha256"],
            source_task["function_uid"],
            source_task["target_class"],
            ...
            raise ValueError(f"V4 round-3 source identity differs: {case_id}")
        terminal = latest_terminals.get(case_id)
        if terminal is None:
            raise ValueError(f"V4 round-3 source terminal is missing: {case_id}")
        expected_status = scope[
            "required_prior_terminal_status_by_case"
        ][case_id]
        if terminal.get("status") != expected_status:
            raise ValueError(f"V4 round-3 source status differs: {case_id}")
        successful: set[str] = set()
        for row in rows:
            if row.get("case_id") != case_id or row.get("status") != "tool_call":
                continue
            name = row.get("tool_name")
            result = row.get("tool_result") or {}
            if "error" in result:
                continue
            if name == "query_angr" and result.get("backend_status") == "unavailable":
                continue
            if name in scope["required_tool_names"]:
                successful.add(name)
        missing = [
            name
            for name in scope["required_tool_names"]
            if name not in successful
        ]
        if missing != scope["required_prior_missing_tool_names_by_case"][case_id]:
            raise ValueError(
                f"V4 round-3 source tool coverage differs: {case_id}"
            )
...
def main() -> int:
    ...
    v4_scope_flags = (
        int(args.reviewed_v4_cve_2021_42374_scope)
        + int(args.reviewed_static_primary_scope)
        + int(args.reviewed_static_required_tools_scope)
        + int(args.reviewed_static_round3_scope)
    )
    if v4_scope_flags > 1:
        raise ValueError("Select only one reviewed v4 scope")
    if v4_scope_flags and args.protocol_version != "v4":
        raise ValueError(
            "The reviewed v4 scope requires protocol v4"
        )
    ...
    elif args.reviewed_static_primary_scope:
        selected_policy_version = V4_STATIC_PRIMARY_POLICY_VERSION
    elif args.reviewed_static_required_tools_scope:
        selected_policy_version = V4_STATIC_REQUIRED_TOOLS_POLICY_VERSION
    elif args.reviewed_static_round3_scope:
        selected_policy_version = V4_STATIC_ROUND3_POLICY_VERSION
    ...
    if args.protocol_version == "v3":
        ...
        if args.reviewed_v4_cve_2021_42374_scope:
            scope_version = V4_CVE_2021_42374_SCOPE_VERSION
        elif args.reviewed_static_primary_scope:
            scope_version = V4_STATIC_PRIMARY_SCOPE_VERSION
        elif args.reviewed_static_required_tools_scope:
            scope_version = V4_STATIC_REQUIRED_TOOLS_SCOPE_VERSION
        elif args.reviewed_static_round3_scope:
            scope_version = V4_STATIC_ROUND3_SCOPE_VERSION
        else:
        ...
        if set(args.case_id) != reviewed_ids or len(args.case_id) != len(
            ...
            if scope_version in {
                ...
                raise ValueError(
                    "V4 static preparation is restricted to its exact "
                    "reviewed three-case scope"
                )
            if scope_version == V4_STATIC_ROUND3_SCOPE_VERSION:
                raise ValueError(
                    "V4 static round-3 preparation is restricted to its "
                    "exact reviewed two-case scope"
                )
            raise ValueError(
    ...
    if reviewed_scope is not None:
        ...
        for case in queue:
            ...
            if scope_version in {
                ...
                case["required_tool_names"] = list(
                    reviewed_scope["required_tool_names"]
                )
        if scope_version == V4_STATIC_ROUND3_SCOPE_VERSION:
            validate_static_round3_scope_source(
                reviewed_scope, results_root
            )
        ...
        manifest.append(
            {
                ...
                "required_tool_names": case.get("required_tool_names", []),
                "tool_coverage_enforcement": policy.get(
                    "tool_coverage_enforcement"
                ),
                **(
                    {
                        "request_defensive_reminder": policy[
                            "request_defensive_reminder"
                        ]
                    }
                    if policy.get("request_defensive_reminder")
                    else {}
                ),
                **(
                    {
                        "required_tool_retry_extension": policy[
                            "required_tool_retry_extension"
                        ]
                    }
                    if policy.get("required_tool_retry_extension")
                    else {}
                ),
                ...
                "package_blocks_sha256": package.metadata.get("blocks_sha256"),
                "package_pcode_sha256": package.metadata.get("pcode_sha256"),
                "package_references_sha256": package.metadata.get(
                    "references_sha256"
                ),
                ...
                "package_block_count": package.metadata.get("block_count"),
                "package_pcode_operation_count": package.metadata.get(
                    "pcode_operation_count"
                ),
                "package_reference_count": package.metadata.get(
                    "reference_count"
                ),
                ...
                "max_angr_queries_per_case": policy["limits"][
                    "max_angr_queries_per_case"
                ],
    ...
    run_summary = {
        ...
        "local_static_analysis_cost_usd": 0.0,
        "local_static_analysis_note": (
            "Ghidra facts are precomputed; bounded angr queries run locally "
            "and consume tool-call/time limits but no provider USD."
        ),
        "tool_coverage_enforcement": policy.get(
            "tool_coverage_enforcement"
        ),
        ...
        **(
        ...
        "required_tool_names": (
            reviewed_scope.get("required_tool_names", [])
            if reviewed_scope
            else []
        ),
        "reviewed_scope_version": (
