...
def load_policy_for_protocol(
    ...
    if policy.get("prompt_version") != V4_PROMPT_VERSION:
...
def load_v4_reviewed_scope(
    ...
    if (
        ...
        raise ValueError("V4 reviewed scope differs from the reviewed v3 cohort")
    for case in cases:
        if case != v3_by_id.get(case["case_id"]):
            raise ValueError("V4 reviewed identity differs from the v3 cohort")
