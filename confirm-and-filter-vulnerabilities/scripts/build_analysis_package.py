...
def parse_export(path: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]], list[dict[str, Any]]]:
    ...
    if metadata.get("analysis_source") != "ghidra-program-model":
        raise ValueError("Analysis export is not from the Ghidra program model")
    ...
    return metadata, functions, callsites
...
def function_uid(
    ...
    digest = sha256_text(
        "|".join([artifact_sha256, address, byte_sha256, instruction_sha256])
    )
...
def main() -> int:
    ...
    metadata, analysis_functions, analysis_callsites = parse_export(analysis_export)
    ...
    for address, (source_path, source) in sorted(sources.items()):
        ...
        uid = function_uid(
            ...
            str(fact["normalized_instruction_sha256"]),
        )
    ...
    package = {
        "package_version": "tier-b-filter-package-v1",
        ...
        "unresolved_indirect_callsite_count": sum(
            row["resolution_status"] == "unresolved" for row in callsite_rows
        ),
